...
const MSWCGodownPage = () => {
  ...
  const handleDelete = async (uuid) => {
    if (!window.confirm("Are you sure you want to delete this godown?")) return;
    try {
      const response = await fetch(`${URL}/api/mswcgodown/${uuid}`, { method: "DELETE" });
      if (response.ok) {
        alert("Godown deleted successfully!");
        fetchGodowns();
      } else {
  ...
  const currentRecords = filteredGodowns.slice(indexOfFirstRecord, indexOfLastRecord);
  const totalPages = Math.ceil(filteredGodowns.length / recordsPerPage);
